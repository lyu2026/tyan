...
String.prototype.r=function(f,x){let r=await cordovaFetch(this+`?_vv=${(new Date).vv()}`,{cache:'force-cache'});for(let k of x.split('.'))r=r.then(_=>_[k=='t'?'text':(k=='h'?'html':'json')]());r.then(f)};
